...
MARK = '// MEDICAL_LLM_STACK_V2'

block = r'''
// MEDICAL_LLM_STACK_V2
...
function medicalModelRank(id) {
    ...
    if (id === MEDICAL_MODELS.medgemma15.id.toLowerCase() || /medgemma[-_:]?1\.5[-_:]?4b/.test(id)) return 130;
    if (id === MEDICAL_MODELS.medgemma27mm.id.toLowerCase() || /medgemma[-_:]?27b(?!.*text)/.test(id)) return 120;
    if (id === MEDICAL_MODELS.medgemma27text.id.toLowerCase() || /medgemma[-_:]?27b.*text|medgemma.*text.*27b/.test(id)) return 115;
    if (/medgemma/.test(id)) return 90;
    return 20;
}
...
function medicalResolvePreferred(preferred) {
    ...
    for (const wanted of preferred || []) {
        ...
        // Tolère les alias locaux vLLM/Ollama sans prétendre qu'un modèle absent existe.
        const wn = wanted.toLowerCase().replace(/^google\//, '').replace(/[^a-z0-9]+/g, '');
        const alias = available.find(m => {
            const an = String(m.id || '').toLowerCase().replace(/^google\//, '').replace(/[^a-z0-9]+/g, '');
            return an === wn || (wn.includes('medgemma15') && /medgemma.*15.*4b/.test(an)) ||
                   (wn.includes('27btext') && /medgemma.*27b.*text|medgemma.*text.*27b/.test(an));
        });
...
function medicalAgentProfile(agentKey) {
    // Toutes les lectures expertes actuelles reçoivent des images : leur modèle primaire doit donc être multimodal.
    // Le 27B text est réservé aux étapes purement textuelles (raisonnement/consensus).
    return {
        key: agentKey,
        primary: [MEDICAL_MODELS.medgemma15.id, MEDICAL_MODELS.medgemma27mm.id],
        reasoning: [MEDICAL_MODELS.medgemma27text.id, MEDICAL_MODELS.medgemma27mm.id, MEDICAL_MODELS.medgemma15.id],
        crosscheck: [MEDICAL_MODELS.medgemma27mm.id, MEDICAL_MODELS.medgemma15.id],
...
# Replace V1 if an earlier generated file is reprocessed; otherwise insert V2.
if '// MEDICAL_LLM_STACK_V1' in s:
    start = s.index('// MEDICAL_LLM_STACK_V1')
...
# Allow a compatible MedGemma endpoint to expose several models. If /models is unavailable,
...
# A MedGemma custom model is a fallback/entry point, not a command to force every role
...
# Consensus is text-only: prefer the deeper 27B medical reasoning model when available.
...
# Make the verified text-reasoning model selectable as an endpoint fallback.
...
print('Medical LLM stack V2 injected into index.html')
